...
import { Text, StyleSheet, View, SafeAreaView, Linking, ScrollView, VirtualizedList } from "react-native";
import { Card, Divider } from 'react-native-elements';


// Import getNews and other functions
import  { testKeywords, findClosestMatchString, keywordCount, sortAlternativeArray } from './src/components/scratchpad';
import { showResult, showAlternative, getjson, getBiasJSON, returnBiasRating, returnBiasLocation, returnLocalAlternative, getBiasList, findKeywords, getNews, isEmpty, constructJSONForDisplay, getHeadlines, getMyHeadlines } from './src/GetCardContent.js';

const oldheadlines = 'test2'; // getMyHeadlines();
var headlineArray = [];

// Import end 

import MyList from './src/components/headlines';
import { render_FlatList_header } from './src/components/header';
import AltNews from './src/Altnews.js';
...
export default class App extends React.Component {
  constructor(props) {
    ...
    // const oldheadlines = 'test'; // getMyHeadlines();
  }
  ...
  fetchNews() {
    getMyHeadlines()
      // .then(firstheadline => this.setState( {myheadlines } = firstheadline[0] ))
      .then(articles => this.setState({ articles, refreshing: false }))
      // .then(alert('firstheadline: ' + this.state.articles[0] ))
  ...
  handleToUpdate = (someArg) => {
    // this.setState({myheadlines: someArg});
    this.setState(
      {
        refreshing: true
      },
      () => this.setState({myheadlines: someArg, refreshing: false })
    );
    alert('We pass argument from Child to Parent: ' + this.state.myheadlines);
}

  render() {
    // var  handleNewList = this.handleToUpdate;
    return (
      <View>

      <Card>
      <FlatList 
        ListHeaderComponent={() => {return (<Text>Top Headlines</Text>)}}
        data={this.state.articles}
        renderItem={({ item}) => <MyList headline = {item} handle = {this.handleToUpdate} />}
      ...
      <Card>
      <AltNews input={this.state.myheadlines}/> 
      </Card>
      </View>
    );
  }
...
//input={this.state.myheadlines} 
// <AltNews input={this.state.myheadlines} handleNewList = {handleNewList.bind(this)}/> 
